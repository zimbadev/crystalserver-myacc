...
if(isset($_POST['war_acpt']) && !empty($_POST['war_acpt'])){
	$acpt_war = $db->query('UPDATE guild_wars SET `status` = '.(int) $status_acpt.' WHERE `id` = '.(int) $_POST['war_acpt']);
}
if(isset($_POST['war_rej']) && !empty($_POST['war_rej'])){
	$acpt_war = $db->query('UPDATE `guild_wars` SET `status` = '.(int) $status_rej.' WHERE `id` = '.(int) $_POST['war_rej']);
}
